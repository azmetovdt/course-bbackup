...
app.get('/', async (_request, response) => {
    ...
    const risk = backupRunner.getRisk(files, backupedFilesMap, 0);
    ...
    if (!backupRunner.hasTimerSet || backupRunner.cachedRisk !== risk) {
        updateTimer(files, backupedFilesMap, 0,)
    }
...
app.post('/runner/sync', async (request, response) => {
    QueryRunner.runSync(backupRunner).subscribe(
        () => response.redirect('/')
    )
});
...
function updateTimer(files: File[], backupedFilesMap: Record<string, BbFile>, faultCount: number) {
    backupRunner.updateTimer(files, backupedFilesMap, faultCount, () => {
